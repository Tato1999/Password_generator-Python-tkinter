...
def save_add_data():
    ...
    new_data = {
        web_name.lower(): {
            "email": mail_name.lower(),
            "password": pass_name.lower()
        }
    }
    # print(web_name)
    # print(mail_name)
...
def search_data():
    ...
    try:
        with open("password.json", mode="r") as data:
            ...
            result_pass_input.insert(0,"Pass: " + data_file[low_search_result_web]['password'])
            search_mail_input.insert(0,"Email: " + data_file[low_search_result_web]['email'])
            search_result_web_show.insert(0,"web: " + search_result_web)
    except FileNotFoundError:
